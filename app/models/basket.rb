class Basket < ActiveRecord::Base
  ...
  def paypal_url(return_url, notify_url)
    values = {
        ...
        :email => user.email,
        :first_name => billing_first_names,
        :last_name => billing_surname,
        :zip => billing_postcode,
        :custom => id
    }
    # calculate the total discount and apply it to the cart
    values = values.merge(:discount_amount_cart => discount_subtotal) if discount_subtotal > 0
    count = 0
    ...
    if gift_wrap?
      count += 1
      values.merge!({
        "amount_#{count}" => sprintf('%.2f', gift_wrap_subtotal),
        "item_name_#{count}" => "Gift Wrap"
      })
    end

    if delivery_subtotal > 0
      count += 1
      values.merge!({
        "amount_#{count}" => sprintf('%.2f', delivery_subtotal),
        "item_name_#{count}" => delivery_summary
      })
    end

    #@paypal_url = "https://www.sandbox.paypal.com/cgi-bin/webscr?" + values.to_query # DEMO
    "https://www.paypal.com/cgi-bin/webscr?" + values.to_query # LIVE
  end
